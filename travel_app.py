# --- 版本：v36 (Google Sheets 雲端多人連線協作版 - 方案 A) ---
...
expected_hotel_cols = ["飯店名稱", "訂房平台", "入住日", "退房日", "地址"] # 移除了檔案欄位
expected_flight_cols = ["航空公司", "航班號碼", "出發地", "抵達地", "起飛時間", "降落時間", "日期"] # 移除了檔案欄位
...
if "data_loaded" not in st.session_state:
    with st.spinner("☁️ 正在與 Google 表單同步資料..."):
        st.session_state.itinerary = load_data_from_gs("行程", expected_itinerary_cols)
        st.session_state.hotels = load_data_from_gs("住宿", expected_hotel_cols)
        st.session_state.flights = load_data_from_gs("航班", expected_flight_cols)
        st.session_state.expenses = load_data_from_gs("記帳", expected_expense_cols)
        st.session_state.exchanges = load_data_from_gs("換匯", expected_exchange_cols)
